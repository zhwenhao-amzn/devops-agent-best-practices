...
> 💡 **Private Connection ≠ 免認證。** Private Connection（透過 VPC Lattice）僅解決網路可達性問題。認證（OAuth/API Key/SigV4）仍然獨立需要。若使用 OAuth + Private Connection，token exchange endpoint 也必須透過同一個 connection 可達。
